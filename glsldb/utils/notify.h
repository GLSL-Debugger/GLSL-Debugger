...
#define UT_NOTIFY(sev, msg)									\
    do {													\
        ...
        str = str + ":" + __func__+"()";                    \
        utils_notify_va(sev, str.c_str(), __LINE__, buf.str().c_str());	\
    }														\
    while(0) 

#else   // C
    #define UT_NOTIFY(sev, ...)                                 \
            utils_notify_va(sev, __FILE__, __LINE__, __VA_ARGS__)
#endif  // __cplusplus
...
extern void utils_notify_va(const severity_t sev, const char* filename, unsigned int line, const char *fmt, ...);
#if defined(__cplusplus)
}
#endif 
#endif  // UT_NOTIFY_H